...
class FynsisRestServiceImpl extends SugarWebServiceImplv4_1
{
    ...
    //functionality may be altered here in response to the key failing to validate
        ...
        $qry=$qry." WHERE (leads.first_name LIKE  '%".$name."%' OR leads.last_name LIKE  '%". $name."%'OR phone_mobile LIKE  '%".$phone."%' OR email.email_address like '%".$email."%') AND leads.deleted=0";
        $result=$db->query($qry);
        $leed_res=array();
        $ids=array();
